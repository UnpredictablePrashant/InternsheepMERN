...
import EmploymentHistory from '../../components/student/EmploymentHistory';
import EducationHistory from '../../components/student/EducationHistory';
...
function Jobmyresume() {
	...
	const { userDetails, updateEducation, isInitialLoaded } = useUser()

	const [resumeData, setResumeData] = useState({
		resume_headline: '', //done
		key_skills: '',
		employment: [],
	...
	function editField(field, value) {
		setResumeData((prevData) => {
			...
			const updatedData = { ...prevData }; // Create a shallow copy of the state
			let current = updatedData; // Pointer to traverse and update the object

			// Traverse the object until the second last key
			for (let i = 0; i < keys.length - 1; i++) {
				if (!current[keys[i]]) {
					current[keys[i]] = {}; // Initialize nested objects if they don't exist
				}
	...
	function deleteEmployment(id) {
		setResumeData((prevData) => ({
			...prevData,
			employment: prevData.employment.filter((job) => job.id !== id), // Remove by ID
		}));
	}
	...
	function addNewEmployment() {
		...
		setResumeData((prevData) => ({
			...prevData,
			employment: [newEmployment, ...prevData.employment], // Add new employment
		}));
	}
	...
	function addNewEducation() {
		...
		setResumeData((prevData) => ({
			...prevData,
			education: [newEducation, ...prevData.education],
		}));
	}
	...
	async function saveEducation() {
		...
		const filteredEducation = education.map(({ id, ...rest }) => rest);

		const res = await updateEducation(filteredEducation)
		if (res) {
			setEducation(false)
			Toast.success('Updated Education')
		} else {
			Toast.failure('Failed To Update Education')
		}
	}
								...
								<div className="col-xl-9 col-lg-8 col-md-8 col-sm-12">
													...
													<div className="modal-body">
														...
														<form>
															<div className="row">
																<div className="col-lg-12 col-md-12">
																	<div className="form-group">
																		<textarea value={resumeData.resume_headline} onChange={(e) => editField('resume_headline', e.target.value)} className="form-control" placeholder="Type Description"></textarea>
																	</div>
																</div>
															</div>
														</form>
									...
									<div id="key_skills_bx" className="job-bx bg-white m-b30">
										...
										<div className="job-time mr-auto">
											...
											{
												resumeData.key_skills.split(',').map((skill) => {
													return (
														<Link key={`key-skill-${skill}`} className="mr-1"><span>{skill}</span></Link>
													)
												})
											}
										</div>
													...
													<div className="modal-body">
														...
														<form>
															<div className="row">
																<div className="col-lg-12 col-md-12">
																	<div className="form-group">
																		<input value={resumeData.key_skills} onChange={(e) => editField('key_skills', e.target.value)} placeholder='HTML, CSS, Javascript' type="text" className="form-control tags_input" />
																	</div>
																</div>
															</div>
														</form>
									...
									<div id="employment_bx" className="job-bx bg-white m-b30 ">
										...
										</div>
										{
											resumeData.employment.length != 0 ?
												<>
													{
														resumeData.employment.map((employment) => (
															...
															>
																...
																>
																	{employment.designation || 'No designation'}
																</p>
																<p
																	style={{
																		marginBottom: '5px',
																...
																>
																	{employment.organisation || 'No organisation specified'}
																</p>
																<p
																	style={{
																		marginBottom: '5px',
																...
																>
																	{employment.from.month && employment.from.year
																		? `${employment.from.month} ${employment.from.year}`
																		: 'From: Not specified'}{' '}
																	-{' '}
																	{employment.to.month && employment.to.year
																		? `${employment.to.month} ${employment.to.year}`
																		: 'To: Not specified'}
																</p>
																<p
																...
																>
																	{employment.currentCompany ? 'Currently working here' : 'Previous employment'}
																</p>
																<p
																	style={{
																		marginBottom: '5px',
																...
																>
																	{employment.description || 'No description provided'}
																</p>
															</div>
														))
													}
												...
												:
												<p>Add Employment History for Recruiters to view it</p>
										}
										...
										<Modal show={employment} onHide={setEmployment} className="modal fade modal-bx-info editor" >
											<div className="modal-dialog my-0" role="document">
												<div className="modal-content">
													<div className="modal-header">
														<h5 className="modal-title" id="EmploymentModalLongTitle">Employment History</h5>
													...
													<div className="modal-body">
														{/* {
															resumeData.employment.map((employment) => {
																...
																const designation = employment.designation
																return (
																	<form>
																		<div className="row">
																			<div className="col-lg-12 col-md-12">
																				<div className="form-group">
																					...
																					<input value={designation} onChange={(e) => editField('employment.designation', e.target.value)} type="text" className="form-control" placeholder="Enter Your Designation" />
																				</div>
																			</div>
																			<div className="col-lg-12 col-md-12">
																				<div className="form-group">
											...
											<div className="modal-dialog my-0" role="document">
												<div className="modal-content">
													...
													<div className="modal-footer">
														...
														<button type="button" onClick={() => saveEducation()} className="site-button">Save</button>
													</div>
												</div>
											</div>
										</Modal>
												...
												<>
													{resumeData.education.map((education) => (
														...
														>
															...
															>
																{education.educationLevel || 'No education level specified'}
															</p>
															{/* <p
																style={{
																	marginBottom: '5px',
															...
															>
																{education.institute || 'No institute specified'}
															</p>
															...
															>
																{education.startYear || 'No year specified'} -
																{education.endYear || 'No year specified'}
															</p>
														</div>
													))}
												</>
											) : (
												<p>Add Education History for Recruiters to view it</p>
											)
										}
									...
									<div id="projects_bx" className="job-bx bg-white m-b30">
										...
										</div>
										{
											resumeData.projects.length !== 0 ? (
												<>
													{resumeData.projects.map((project) => (
														...
														>
															...
															>
																{project.from.month && project.from.year
																	? `${project.from.month} ${project.from.year}`
																	: 'From: Not specified'}{' '}
																-{' '}
																{project.to.month && project.to.year
																	? `${project.to.month} ${project.to.year}`
																	: 'To: Not specified'}
															</p>
															<p
												...
												<p>Add Project History for Recruiters to view it</p>
											)
										}
															...
															<div className="row">
																<div className="col-lg-12 col-md-12">
																	<div className="form-group">
																		...
																		<textarea value={resumeData.profile_summary} onChange={(e) => editField('profile_summary', e.target.value)} className="form-control" placeholder=""></textarea>
																	</div>
																</div>
															</div>
														</form>
